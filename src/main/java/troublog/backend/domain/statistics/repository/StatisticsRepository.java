...
public interface StatisticsRepository extends JpaRepository<Post, Long> {
	...
	@Query("""
			SELECT new troublog.backend.domain.statistics.dto.response.StatsResDto(t.name, COUNT(t))
			FROM PostTag pt JOIN pt.post p JOIN pt.tag t
			WHERE p.user.id = :userId AND t.tagType = :tagType
			GROUP BY t.name
			ORDER BY COUNT(t) DESC
			""")
	List<StatsResDto> findTopTagsByUser(@Param("userId") Long userId, @Param("tagType") TagType tagType);

	@Query("""
			SELECT new troublog.backend.domain.statistics.dto.response.StatsResDto(
				CAST(ps.summaryType AS string), COUNT(ps)
			)
			FROM PostSummary ps
